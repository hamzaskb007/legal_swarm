...
class ConfidenceScorer:
    def score(self, entry: RegulatoryEntry) -> ConfidenceScore:
        ...
        factors.append(
            f"Base authority score ({entry.source_governance.dominant_source}): {base:.2f}"
        )
        ...
        if unresolved:
            factors.append(
                f"Contradiction penalty ({unresolved} unresolved): -{contradiction_penalty:.2f}"
            )

        level_4_5_count = sum(1 for c in all_citations if getattr(c, "authority_level", 5) >= 4)
        authority_level_penalty = level_4_5_count * AUTHORITY_LEVEL_PENALTY
        if level_4_5_count:
            factors.append(
                f"Authority level penalty ({level_4_5_count} at level 4+): -{authority_level_penalty:.2f}"
            )
        ...
        factors.append(
            f"Module completeness bonus ({populated}/{len(COMPLETENESS_FIELDS)}): +{completeness_bonus:.4f}"
        )
        ...
        if days_since_update > REFRESH_THRESHOLD_DAYS:
            ...
            factors.append(
                f"Refresh recency penalty ({days_since_update}d since update): -{refresh_penalty:.4f}"
            )
        else:
            refresh_penalty = 0.0
