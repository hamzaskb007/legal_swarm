...
class CitationContradictionDetector:
    def detect(self, entry: RegulatoryEntry) -> list[ContradictionRecord]:
        ...
        for citation in (
            entry.source_governance.secondary_citations + entry.source_governance.tertiary_citations
        ):
            if citation.reliability_score > max_primary_score:
                primary_citation = max(
                    entry.source_governance.primary_citations, key=lambda c: c.reliability_score
                )
...
COMPARABLE_FIELDS = [
    ...
    "investor_requirements.qualified_investor_required",
    "investor_requirements.min_investment_usd",
